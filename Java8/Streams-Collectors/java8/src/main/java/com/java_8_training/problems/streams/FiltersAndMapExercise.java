...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter((Integer value) -> (value%2) == 0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter((String nm) -> nm.equals(name)).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter((Integer nr) -> nr!=null).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter((Person x) -> x.getName().equals(name) && x.getAge()==age).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map((Integer i) -> i*2).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
        collect = personList.stream().map((Person prs) -> prs.getName()).collect(toList());
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        //TODO #9
        personOptional = personList.stream().filter((Person x)->x.isMale()).findFirst();
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch((Person x) -> x.getAge() < 70);
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        min = personList.stream().map((Person x) -> x.getAge()).reduce(Integer.MAX_VALUE, Integer::min);
        Person person = personList.stream().collect(minBy(comparingInt(Person::getAge))).get();
        min = person.getAge();
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter((Person p) -> p.getName().startsWith("J"))
                .map(Person::getNationality).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().map((Integer i) -> i*i).filter(i -> i%3 == 0).findFirst();
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().map(Person::getSalary).reduce(0, (acc, x) -> acc + x);
